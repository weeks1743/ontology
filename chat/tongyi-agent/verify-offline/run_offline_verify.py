...
def is_task_done(payload: Dict[str, Any]) -> bool:
    ...
    if output.get("status") in (0, "0"):
        return True
    # Some intermediate polling responses expose only partial assets
    # (for example transcriptionPath) before the final summaries and chapters
    # are generated. Returning early here causes incomplete downloads.
    return False
