...
> 图片占位 2：逻辑行为工作区。建议放一张行为与归属对象、输入、规则引用、事件引用的界面图。
...
> 图片占位 3：约束规则工作区。建议放一张规则表达式、失败提示、适用对象/行为的界面图。
...
> 图片占位 4：消息事件工作区。建议放一张事件的生产者、订阅者、影响对象的界面图。
...
> 图片占位 5：CRM 本体五类核心资产总览。建议放一张能同时体现对象、行为、规则、事件、场景关系的图。
>
> 图片占位 6：Palantir Ontology / Action / Operational App 参考图。建议放一张说明“本体不是静态模型，而是动作与应用的源头”的图。
...
> 图片占位 7：本体工作台总览。建议放一张包含左侧导航和七个工作区入口的界面图。
...
> 图片占位 8：引用校验或删除拦截弹窗。建议放一张对象被引用时无法直接删除的界面图。
>
> 图片占位 9：YAML/Validation 工作区。建议放一张展示错误、警告和校验结果的界面图。
...
> 图片占位 10：YAML 六件套视图。建议放一张同时展示 `model.yaml`、`objects.yaml`、`behaviors.yaml` 等标签页的界面图。
...
> 图片占位 11：可视化图谱工作区。建议放一张对象、行为、规则、事件、场景混合展示的拓扑图。
>
> 图片占位 12：场景模拟过程。建议放一张场景逐步高亮执行路径的界面图。
...
> 图片占位 13：对象或行为编辑表单。建议放一张能体现属性类型、关系、规则引用、事件引用等结构化字段的界面图。
