...
如果某个自定义主题不希望显示导出界面元素，只需在模板的 `<body>` 上加 `data-export-progress="false"`。这一个开关会同时隐藏顶部进度条和右侧导航点，并在通过 `kai-html-export` 做 native 模式导出时保持一致。
...
`references/design-quality.md`（在 `--generate` 阶段与风格文件一起加载）收录了一套规则，强制模型根据**内容密度**做布局决策，而不仅仅是内容类型：
...
**内容语调配色校准。** 规划模板现在根据内容类型推荐语调匹配的强调色，让配色系统强化内容的情感基调：
